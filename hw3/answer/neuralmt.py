...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        ######## Implementation 1: Fix Attention########
        # Additive attention: compute relevance scores between decoder and each encoder state 
        ################################################
        enc_proj = self.W_enc(encoder_out)     
        dec_proj = self.W_dec(decoder_hidden)   
        score = torch.tanh(enc_proj + dec_proj) 

        # Apply V_att to get scalar scores
        score = self.V_att(score)         
        alpha = torch.nn.functional.softmax(score, dim=0)  
        return alpha
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        ######## Implementation 1: Fix Attention########
        # Compute context vector as weighted sum of encoder states using attention weights 
        ################################################
        alpha = self.calcAlpha(decoder_hidden, encoder_out)  
...
######## Implementation 2: Beam Search ########
# Add beam search function
###############################################
...
def beamSearchDecoder(decoder, encoder_out, encoder_hidden, maxLen, beam_width=5):
    ...
    for t in range(maxLen):
        ...
        for score, seq, hidden in beams:
            ...
            log_probs = torch.nn.functional.log_softmax(output.squeeze(0).squeeze(0), dim=0)
    ...
    # Return best sequence (length normalized)
    completed.sort(key=lambda x: x[0] / len(x[1]), reverse=True)
    best_seq = completed[0][1]
    
    # Convert to output format
    target_vocab_size = decoder.target_vocab_size
    outputs = torch.zeros(len(best_seq), 1, target_vocab_size, device=device)
    for i, token in enumerate(best_seq):
        outputs[i, 0, token] = 1.0
    
    alphas = torch.zeros(1, seq1_len, len(best_seq))
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, maxLen=None,use_beam=True, beam_width=5):
        """
        ...
        #### Implementation 2: Beam Search ####
        if use_beam:
            return beamSearchDecoder(self.decoder, encoder_out, encoder_hidden,
                                  maxLen, beam_width)
        else:
